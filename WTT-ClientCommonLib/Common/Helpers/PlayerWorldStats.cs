...
public class PlayerWorldStats(ManualLogSource logger)
{
    ...
    {
        logger.LogDebug($"{playerType} Position X: {player.Transform.position.x} Y: {player.Transform.position.y} Z: {player.Transform.position.z}");
        logger.LogDebug($"{playerType} Rotation X: {player.gameObject.transform.rotation.eulerAngles.x} Y: {player.gameObject.transform.rotation.eulerAngles.y} Z: {player.gameObject.transform.rotation.eulerAngles.z}");
    }
}