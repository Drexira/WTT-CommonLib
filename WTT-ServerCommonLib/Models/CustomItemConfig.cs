...
namespace WTTServerCommonLib.Models
{
    public class CustomItemConfig
    {
        ...
        public required int FleaPriceRoubles { get; set; }
                ...
                foreach (var traderEntry in Traders)
                {
                    ...
                    foreach (var schemeEntry in schemes)
                    {
                        ...
                        if (scheme.Barters == null || scheme.Barters.Count == 0)
                            throw new InvalidDataException($"traders['{traderKey}']['{schemeKey}'] must include at least one barter");

                        for (var i = 0; i < scheme.Barters.Count; i++)
                        {
                            var barter = scheme.Barters[i];
                            if (barter == null)
                                throw new InvalidDataException($"traders['{traderKey}']['{schemeKey}'].barters[{i}] is null");

                            if (string.IsNullOrWhiteSpace(barter.Template))
                                throw new InvalidDataException($"traders['{traderKey}']['{schemeKey}'].barters[{i}].template is required");

                            if (barter.Count <= 0)
                                throw new InvalidDataException($"traders['{traderKey}']['{schemeKey}'].barters[{i}].count must be > 0");
